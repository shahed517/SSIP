...
class GradLogPEstimator2d(BaseModule):
    def __init__(self, dim, dim_mults=(1, 2, 4, 8), groups=8,
                # n_spks=None, spk_emb_dim=64, n_feats=80, 
                pe_scale=1000):
        ...
        # since we are not using speaker embeddings, we can simplify the above line to:
        dims = [1, *map(lambda m: dim * m, dim_mults)]  
        # Spectrograms are treated as single channels images, 
        # - channels is set to 1
        # - n_feats become the height
        # - time is the width
        # - batch example: (batch_size, 1, n_feats, time)
...
class Diffusion(BaseModule):
    def __init__(self, 
        ...
        self.estimator = GradLogPEstimator2d(dim, 
                                            #  n_spks=n_spks, spk_emb_dim=spk_emb_dim,
                                             pe_scale=pe_scale)
    ...
    @torch.no_grad()
    def reverse_diffusion(self, z, n_timesteps, stoc=False):
        h = 1.0 / n_timesteps
        xt = z #* mask
    ...
    @torch.no_grad()
    def denoise(self, z, n_timesteps, start_t, stoc=False):
        """Denoise input x starting from start_t for n_timesteps."""
    ...
    @torch.no_grad()
    def forward(self, z, n_timesteps, stoc=False):
        return self.reverse_diffusion(z, n_timesteps, stoc)
